...
class FileButtonField extends Component {
  ...
  handleInputChange = onChange => (event) => {
    ...
    uploadHook(formData)
      ...
      .catch((error) => {
        this.setState({ error }, () => {
          // в качестве value пердается что то
          // это необходимо для вызова validate
          onChange(uniq());
        });
      });
  };
  ...
  render = () => {
    const {
      disabled, classes, accept, uploadHook, ...props
    } = this.props;
      ...
      <Field {...props} validate={this.validate}>
        {({ input: { onChange, onBlur, onFocus } }) => (
          <Fragment>
            ...
            >
              <svg width="16" height="14" viewBox="0 0 16 14" fill="currentColor" style={{ marginRight: '10px' }}>
