...
describe('`mustBePassportNumber` validation rule', () => {
  test('should return an error object if value is invalid', () => {
    expect(mustBePassportNumber(12345678)).toEqual('Неверный номер. Введите корректный номер.');
    expect(mustBePassportNumber(121)).toEqual('Неверный номер. Введите корректный номер.');
  });
